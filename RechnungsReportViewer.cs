...
{
    public class RechnungsReportViewer : MyForm
    {
        ...
        protected override void _InitializeComponent()
        {
            ...
            report.LocalReport.ReportEmbeddedResource = "FCC_Verwaltungssystem.Rechnung_Report.rdlc";
        ...
        public void mailReport(string rechnungsnummer)
        {
            getData(rechnungsnummer);
            sendMessage();
            Close();
        }
        public void PrintReport(string rechnungsnummer)
        {
            ...
            try
            {
                ...
                string fileName = Globals.FORMULAR_RECHNUNG + "_" + rechnungsnummer + ".pdf";
                FilePath = saveAsPDF(fileName);
                updateRechnungStatus(Prozessstatus.GEDRUCKT);
                Close();
            }
            catch (Exception exp)
            {
        ...
        public void PreviewReport(string rechnungsnummer)
        {
            getData(rechnungsnummer);
            Hide();
        ...
        public void getData(string rechnungsnummer)
        {
            leistungsdaten = DataAccessLayer.getLeistungenByRechnungsnummer(rechnungsnummer);
            rechnungsdaten = DataAccessLayer.getRechnungByNummer(rechnungsnummer);
        ...
        private void ReportViewer_Load()
        {
            try
            {
                ...
                var rechnungsnummer = (string)row["RECHNUNGSNUMMER"];
                var leistugsbeginn = (DateTime)row["LEISTUNGSBEGINN"];
                var leistugsende = (DateTime)row["LEISTUNGSENDE"];
                var rechnungsdatum = (DateTime)row["RECHNUNGSDATUM"];
                var bezahlt = (bool)row["BEZAHLT"];
                var waehrung = (string)row["WAEHRUNG"];
                kundenMail = (string)row["EMAIL"];
                //date_Druckdatum.Value = (DateTime)row["DRUCKDATUM"];
                ...
                if (setting != null && setting.Rows.Count > 0)
                {
                    ...
                    kText = rowf.Field<string>("KOPFTEXT");
                    fText = rowf.Field<string>("FUSSTEXT");
                    zahltext = rowf.Field<string>("ZAHLUNGSTEXT");
                    zusatztext = rowf.Field<string>("ZUSATZTEXT");
            ...
            new ReportParameter("Rechnungsnummer",rechnungsnummer),
            new ReportParameter("Leistungsbeginn",leistugsbeginn.Date.ToString("dd-MM-yyyy")),
            new ReportParameter("Leistungsende",leistugsende.Date.ToString("dd-MM-yyyy")),
            new ReportParameter("Zahlungsziel",zahlungsziel.Date.ToString("dd-MM-yyyy")),
            new ReportParameter("Rechnungsdatum",rechnungsdatum.Date.ToString("dd-MM-yyyy")),
            new ReportParameter("Bezahlt",bezahlt.ToString()),
            new ReportParameter("WAEHRUNG",waehrung),
            new ReportParameter("Anrede",anrede),
            new ReportParameter("Kopftext",kText),
            new ReportParameter("Fusstext",fText),
            new ReportParameter("Zahlungsziel_Text",zahltext),
        ...
        private void sendMessage()
        {
            ...
            var rechnungsnummer = (string)r_row["RECHNUNGSNUMMER"];
            ...
            if (dataTable != null && dataTable.Rows.Count > 0)
            {
                ...
                if (aktive_Einstellung.Equals("OUTLK"))
                {
                    ...
                    mailItem.Subject = "Rechnung - Fight Club Chemnitz";
                    ...
                    updateRechnungStatus(Prozessstatus.VERSANDT);
                }
                else if (aktive_Einstellung.Equals("SMTP"))
                {
                    ...
                    eMailData.Betreff = "Rechnung - Fight Club Chemnitz";
                    eMailData.Body = string.IsNullOrEmpty(zusatztext)? "": zusatztext.Replace("<ANREDE>", anrede);
        ...
        private string saveAsPDF(string fileName)
        {
            ...
            {
                DataRow rowf = setting.Rows[0];
                standardpath = Path.GetFullPath(rowf.Field<string>("ABLAGEORT"));
        ...
        private void updateRechnungStatus(Prozessstatus newstatus)
        {
            DataRow r_row = rechnungsdaten.Rows[0];
            var rechnungsnummer = (string)r_row["RECHNUNGSNUMMER"];

            // Status der Rechnung updaten
            DataAccessLayer.update_RechnungStatus(rechnungsnummer, newstatus);
            //Log
            Log.Information("Rechnung wurde gedruckt -> \n" + JsonConvert.SerializeObject(rechnungsdaten) +
                "\n" + JsonConvert.SerializeObject(leistungsdaten));
        }
    }
}
